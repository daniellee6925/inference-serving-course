...
You've now built the entire single-node serving path — from one forward pass (Ch.01) through the batched, paged, cached, constrained, long-context loop (Ch.02–14). The next block leaves the single process behind. Ch.15 is **backend infrastructure**: how this engine becomes a *service* — API servers, request queues, worker pools, multi-replica scaling, multi-tenancy, and the gateway that fronts them. It's where the fast engine you've built meets real traffic, real users, and the operational reality of running it around the clock.
